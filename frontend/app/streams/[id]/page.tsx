...
const STREAM_ADVANCED_PARAMS: ParameterConfig[] = [
  {
    key: "business_service_flag",
    label: "Business Service",
    type: "boolean",
    category: "advanced",
  },
  {
    key: "enable_stream_run_cancelation",
    label: "Abbruch erlaubt",
    type: "boolean",
    category: "advanced",
  },
  {
    key: "concurrent_stream_runs_enabled",
    label: "Parallel-Lauf erlaubt",
    type: "boolean",
    category: "advanced",
  },
  {
  ...
  {
    key: "automatic_prepared_runs",
    label: "Auto. Prepared Runs",
    type: "number",
    category: "advanced",
  },
  {
    key: "auto_preparation_type",
    label: "Auto. Prep. Type",
    type: "select",
...
const SYSTEM_PARAMS: ParameterConfig[] = [
  {
    ...
    type: "text",
    category: "system",
    helperText: "z.B. /Abteilung/Team",
  },
  {
    key: "master_stream_id",
    label: "Master Stream ID",
    type: "text",
    category: "system",
  },
  { key: "stream_version", label: "Version", type: "text", category: "system" },
  {
    key: "stream_version_type",
    label: "Versions-Typ",
    type: "select",
    options: ["Current", "Draft"],
    category: "system",
  },
  {
    key: "auto_deployment_status",
    label: "Auto-Deploy Status",
    type: "select",
    options: ["Finished", "Released"],
    category: "system",
  },
  {
    key: "schedule_rules_merge_type",
    label: "Sched. Merge Typ",
    type: "select",
    options: ["FromNew", "Merge"],
    category: "system",
  },
];
...
const DEPENDENCY_PARAMS: ParameterConfig[] = [
  {
    key: "external_dependency_stream",
    label: "Ext. Abhängigkeit (Stream)",
    type: "text",
    category: "dependency",
  },
  {
    key: "external_dependency_job",
    label: "Ext. Abhängigkeit (Job)",
    type: "text",
    category: "dependency",
  },
  {
    key: "external_dependency_type",
    label: "Typ",
    type: "select",
    options: ["Success", "Failure", "Complete"],
    category: "dependency",
  },
  {
    key: "file_dependency_path",
    label: "Datei-Trigger Pfad",
    type: "text",
    category: "dependency",
  },
  {
    key: "file_dependency_condition",
    label: "Bedingung",
    type: "select",
    options: ["Exists", "Missing"],
    category: "dependency",
  },
  {
    key: "file_dependency_interval",
    label: "Prüfintervall (sek)",
    type: "number",
    category: "dependency",
  },
];
...
const ALL_STANDARD_JOB_PARAMS: ParameterConfig[] = [
  ...
  },
  {
    ...
    label: "Script-Typ",
    type: "select",
    options: ["Windows", "Unix", "Python", "PowerShell", "Lua", "Shell"],
    category: "job",
  },
  {
    key: "main_script",
  ...
  {
    key: "job_severity_group",
    label: "Severity Gruppe",
    type: "text",
    category: "job",
  },
  {
...
const ALL_FT_JOB_PARAMS: ParameterConfig[] = [
  ...
  },
  {
    ...
    label: "Quelldatei löschen",
    type: "boolean",
    category: "job",
  },
  {
    key: "source_login_object",
    label: "Quelle Login",
    type: "text",
    category: "job",
  },
  {
    key: "target_login_object",
    label: "Ziel Login",
    type: "text",
    category: "job",
  },
...
const ALL_SCHEDULE_PARAMS: ParameterConfig[] = [
  {
    ...
    label: "Frequenz",
    type: "select",
    options: ["einmalig", "täglich", "wöchentlich", "monatlich"],
    category: "schedule",
  },
...
const ALL_CONTACT_PARAMS: ParameterConfig[] = [
  ...
  { key: "department", label: "Abteilung", type: "text", category: "contact" },
  {
    ...
    label: "Kontakt Rolle",
    type: "select",
    options: ["Owner", "Notification", "None"],
    category: "contact",
  },
  {
    key: "notify_on_error",
    label: "Benachrichtigung bei Fehler",
    type: "select",
    options: ["YES", "NO"],
    category: "contact",
  },
...
export default function StreamDetailPage() {
  ...
  const loadSession = async () => {
    ...
    try {
      const res = await fetch(
        `http://localhost:8000/api/wizard/sessions/${sessionId}`,
      );
      if (res.ok) {
        const data = await res.json();
        let loadedParams = data.params;

        // Apply changes from URL parameters (from command execution)
        const applyAction = searchParams.get("apply");
        if (applyAction) {
          const changesApplied: { key: string; value: string }[] = [];

          // Handle different apply actions
          if (applyAction === "rename") {
            const newName = searchParams.get("stream_name");
            if (newName) {
              loadedParams = { ...loadedParams, stream_name: newName };
              changesApplied.push({ key: "stream_name", value: newName });
            }
          } else if (applyAction === "param") {
            // Apply any parameter from URL
            searchParams.forEach((value, key) => {
              if (key !== "apply" && key !== "session_id") {
                loadedParams = { ...loadedParams, [key]: value };
                changesApplied.push({ key, value });
              }
            });
          }
          ...
          if (changesApplied.length > 0) {
            setAppliedChanges(changesApplied[0]);
            // Clear URL params but keep on page
            router.replace(`/streams/${sessionId}`, { scroll: false });
            // Auto-clear notification after 5 seconds
            setTimeout(() => setAppliedChanges(null), 5000);
          }
        }

        setSession({
          id: sessionId,
          name: loadedParams.stream_name || "Entwurf",
          status: data.status || "complete",
          params: loadedParams,
          job_type: data.detected_job_type || "STANDARD",
        });
        setEditedParams(loadedParams);
        validateParams(loadedParams, data.detected_job_type || "STANDARD");
        // Generate initial XML preview
        generateXmlPreview(loadedParams, data.detected_job_type);
      }
    } catch (error) {
  ...
  const validateParams = (
    ...
    if (jobType === "STANDARD")
      required.push("agent_detail", "script_type", "main_script");
    if (jobType === "FILE_TRANSFER")
      required.push(
        "source_agent",
        "target_agent",
        "source_file_pattern",
    ...
    if (jobType === "SAP")
      required.push("sap_system", "sap_client", "sap_report");
  ...
  const generateXmlPreview = async (
    paramsToUse: Record<string, unknown>,
    jobType?: string,
  ) => {
  ...
  const renderParamInput = (config: ParameterConfig) => {
    ...
    const hasError =
      !value &&
      validationErrors.some((e) => e.includes(config.label.toUpperCase()));
    ...
    const baseClass = cn(
      ...
      hasError
        ...
        : "border-gray-200 focus:border-[#0082D9]",
    );
    ...
    return (
      <div className="space-y-3 pb-4">
        ...
        >
          {title}
          {isSystem && (
            <span className="text-[8px] bg-purple-100 text-purple-600 px-1 rounded">
              SYSTEM
            </span>
          )}
        </h4>
            ...
            <div key={config.key}>
              <div className="flex justify-between items-baseline mb-1">
                <label className="text-[10px] font-medium text-gray-600 flex items-center gap-1">
                  {config.label}
                  {["stream_name", "short_description"].includes(
                    config.key,
  ...
  return (
    <AppLayout sessionId={sessionId}>
      ...
      {/* Applied Changes Notification */}
      {appliedChanges && (
        <div className="bg-gradient-to-r from-blue-50 to-emerald-50 border-b border-blue-100 px-4 py-2 flex items-center justify-between">
          <div className="flex items-center gap-2 text-sm">
            <Sparkles className="w-4 h-4 text-blue-500" />
            <span className="text-blue-700">
              <strong>
                {appliedChanges.key === "stream_name"
                  ? "Name"
                  : appliedChanges.key}
              </strong>{" "}
              wurde auf{" "}
          ...
          <button
            onClick={() => setAppliedChanges(null)}
                  ...
                  {validationErrors.length > 0 ? (
                    <div className="space-y-2">
                      {validationErrors.map((err, i) => (
                        <div
                          key={i}
                          className="flex gap-2 p-2 rounded bg-amber-50 border border-amber-100"
                        >
                  ...
                  <div className="flex-1 relative min-h-0">
                    <textarea
                      ...
                      onKeyDown={(e) =>
                        e.key === "Enter" &&
                        !e.shiftKey &&
                        (e.preventDefault(), handleAiInput())
                      }
        ...
        <div className="flex-[3] flex flex-col bg-white min-w-[360px] border-l border-gray-200 shadow-[0_0_15px_rgba(0,0,0,0.02)] relative z-20">
          <div className="px-5 py-3 border-b border-gray-100 flex items-center justify-between bg-white bg-opacity-90 backdrop-blur-sm sticky top-0 z-10">
            ...
            <button
              ...
              title={
                showSystemParams
                  ? "System-Parameter verbergen"
                  : "Alle Parameter anzeigen anzeigen"
              }
            >
              {showSystemParams ? (
