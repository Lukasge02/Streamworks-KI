...
export default function WizardStep2Contact({
  session,
  onSave,
  isLoading,
}: Props) {
  const [firstName, setFirstName] = useState(
    (session?.params.contact_first_name as string) || "",
  );
  const [lastName, setLastName] = useState(
    (session?.params.contact_last_name as string) || "",
  );
  const [company, setCompany] = useState(
    (session?.params.company_name as string) || "",
  );
  const [department, setDepartment] = useState(
    (session?.params.department as string) || "",
  );

  useEffect(() => {
    if (session?.params.contact_first_name)
      setFirstName(session.params.contact_first_name as string);
    if (session?.params.contact_last_name)
      setLastName(session.params.contact_last_name as string);
    if (session?.params.company_name)
      setCompany(session.params.company_name as string);
    if (session?.params.department)
      setDepartment(session.params.department as string);
  }, [session?.params]);
