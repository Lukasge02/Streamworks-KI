...
export default function WizardStep5Schedule({
  session,
  onSave,
  isLoading,
}: Props) {
  const [schedule, setSchedule] = useState(
    (session?.params.schedule as string) || "daily",
  );
  const { options: calendarOptions } = useOptions("calendar");
  const [startTime, setStartTime] = useState(
    (session?.params.start_time as string) || "08:00",
  );
  const [calendarId, setCalendarId] = useState(
    (session?.params.calendar_id as string) || "",
  );

  useEffect(() => {
    if (session?.ai_suggestions?.schedule) {
      const suggestion = session.ai_suggestions.schedule.toLowerCase();
      if (suggestion.includes("täglich") || suggestion.includes("daily"))
        setSchedule("daily");
      if (suggestion.includes("wöchentlich") || suggestion.includes("weekly"))
        setSchedule("weekly");
      if (suggestion.includes("monatlich") || suggestion.includes("monthly"))
        setSchedule("monthly");
      if (suggestion.includes("stündlich") || suggestion.includes("hourly"))
        setSchedule("hourly");
    }
