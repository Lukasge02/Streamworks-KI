...
export default function WizardStep4JobDetails({
  session,
  onSave,
  isLoading,
}: Props) {
  const jobType =
    ...
    (session?.params.job_type as string) ||
    "STANDARD";
  ...
  const [agentDetail, setAgentDetail] = useState(
    (session?.params.agent_detail as string) || s?.agent_detail || "",
  );
  const [mainScript, setMainScript] = useState(
    (session?.params.main_script as string) || "",
  );
  const [scriptType, setScriptType] = useState(
    (session?.params.script_type as string) || "Windows",
  );

  // File Transfer fields
  const [sourceAgent, setSourceAgent] = useState(
    (session?.params.source_agent as string) || s?.source_agent || "",
  );
  const [targetAgent, setTargetAgent] = useState(
    (session?.params.target_agent as string) || s?.target_agent || "",
  );
  const [sourceFile, setSourceFile] = useState(
    (session?.params.source_file_pattern as string) ||
      s?.source_file_pattern ||
      "",
  );
  const [targetPath, setTargetPath] = useState(
    (session?.params.target_file_path as string) || s?.target_file_path || "",
  );
  const [sourceLogin, setSourceLogin] = useState(
    (session?.params.source_login_object as string) || "",
  );
  const [targetLogin, setTargetLogin] = useState(
    (session?.params.target_login_object as string) || "",
  );

  // SAP fields
  const [sapSystem, setSapSystem] = useState(
    (session?.params.sap_system as string) || s?.sap_system || "",
  );
  const [sapClient, setSapClient] = useState(
    (session?.params.sap_client as string) || "",
  );
  const [sapReport, setSapReport] = useState(
    (session?.params.sap_report as string) || s?.sap_report || "",
  );
  const [sapVariant, setSapVariant] = useState(
    (session?.params.sap_variant as string) || "",
  );
  ...
  const handleContinue = async () => {
    ...
    if (jobType === "STANDARD") {
      ...
      data = {
        ...
        source_file_pattern: sourceFile,
        target_file_path: targetPath,
        source_login_object: sourceLogin || null,
        target_login_object: targetLogin || null,
      };
    } else if (jobType === "SAP") {
      data = {
        sap_system: sapSystem,
  ...
  const isValid = () => {
    ...
    if (jobType === "FILE_TRANSFER")
      return sourceAgent.trim() && targetAgent.trim() && sourceFile.trim();
  ...
  const descriptions: Record<string, string> = {
    ...
    FILE_TRANSFER:
      "Definieren Sie Quell- und Zielserver für den Dateitransfer.",
