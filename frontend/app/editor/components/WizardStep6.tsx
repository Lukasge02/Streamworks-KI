...
export default function WizardStep6Preview({
  session,
  onSave,
  onGenerate,
  isLoading,
  ...
  return (
    <div>
      ...
      <p style={{ color: "#6b7280", marginBottom: "1rem" }}>
        Überprüfen Sie Ihre Eingaben und wechseln Sie zur Detail-Ansicht für die
        finale Generierung.
      </p>
        ...
        >
          ...
          >
            <div style={{ display: "flex", justifyContent: "space-between" }}>
              <span style={{ color: "#6b7280" }}>Name:</span>
              <span style={{ fontWeight: 500 }}>
                {session?.params.contact_first_name}{" "}
                {session?.params.contact_last_name}
              </span>
            </div>
        ...
        >
          ...
          >
            <div style={{ display: "flex", justifyContent: "space-between" }}>
              ...
              <span style={{ fontWeight: 500 }}>
                {session?.params.schedule || "täglich"}
              </span>
            </div>
        ...
        >
          ...
          >
            {jobType === "STANDARD" && (
              <>
                <div>
                  ...
                  {session?.params.agent_detail}
                </div>
                <div>
                  <span style={{ color: "#6b7280" }}>Script:</span>{" "}
                  {(session?.params.main_script as string)?.substring(0, 30)}...
                </div>
              </>
            )}
            {jobType === "FILE_TRANSFER" && (
              <>
                <div>
                  <span style={{ color: "#6b7280" }}>Von:</span>{" "}
                  {session?.params.source_agent}
                </div>
                <div>
                  <span style={{ color: "#6b7280" }}>Nach:</span>{" "}
                  {session?.params.target_agent}
                </div>
              </>
            )}
            {jobType === "SAP" && (
              <>
                <div>
                  ...
                  {session?.params.sap_system}
                </div>
                <div>
                  <span style={{ color: "#6b7280" }}>Report:</span>{" "}
                  {session?.params.sap_report}
                </div>
              </>
            )}
          </div>
