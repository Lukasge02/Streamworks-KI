...
export default function WizardStep1BasicInfo({
  session,
  onSave,
  onAnalyze,
  isLoading,
  ...
  const [streamName, setStreamName] = useState(
    (session?.params.stream_name as string) || "",
  );
  const [description, setDescription] = useState(
    (session?.params.short_description as string) || "",
  );
  ...
  useEffect(() => {
    if (session?.params.stream_name)
      setStreamName(session.params.stream_name as string);
    if (session?.params.short_description)
      setDescription(session.params.short_description as string);
  }, [session?.params]);
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      if (
        streamName &&
        (streamName !== session?.params.stream_name ||
          description !== session?.params.short_description)
      ) {
        onSave(
          "basic_info",
          {
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-500 text-sm">
          Geben Sie Ihrem Stream einen Namen und beschreiben Sie, was er tun
          soll.
        </p>
      </div>
        ...
        {analysisResult && (
          ...
          >
            ...
            <div className="p-4 space-y-4">
              ...
              {analysisResult.suggested_params &&
                    ...
                    <div className="grid gap-2">
                      {Object.entries(analysisResult.suggested_params)
                        ...
                        .map(([key, value]) => {
                          ...
                          const isExplicit = description
                            .toLowerCase()
                            .includes(
                              String(value).toLowerCase().substring(0, 4),
                            );
                          const paramConfidence = isExplicit
                            ? "high"
                            : "medium";
