...
function EditorPageContent() {
  ...
  const fetchSession = async (id: string) => {
    ...
    try {
      const res = await fetch(
        `http://localhost:8000/api/wizard/sessions/${id}`,
      );
      if (res.ok) {
  ...
  const createSession = async () => {
    ...
    try {
      ...
      setError(
        "Backend nicht erreichbar. Starte es mit: cd backend && python main.py",
      );
    } finally {
      setIsLoading(false);
    }
  ...
  const analyzeDescription = async (description: string) => {
    try {
      ...
      // Create session on first analyze (lazy creation) - same as saveStepData
      if (!currentSession) {
        const createResponse = await fetch(
          "http://localhost:8000/api/wizard/sessions",
          {
      ...
      const response = await fetch(
        `http://localhost:8000/api/wizard/sessions/${currentSession!.session_id}/analyze`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
      ...
      const sessionResponse = await fetch(
        `http://localhost:8000/api/wizard/sessions/${currentSession!.session_id}`,
      );
  ...
  const sendChatMessage = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setChatMessages((prev) => [
          ...prev,
          { role: "assistant", content: "Fehler bei der Verarbeitung." },
        ]);
      }
    } catch {
      setChatMessages((prev) => [
        ...
        {
          role: "assistant",
          content: "Verbindung zum Backend fehlgeschlagen.",
        },
      ]);
    } finally {
        ...
        <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
          ...
          <div className="flex items-center justify-between mb-3 flex-shrink-0">
            <span className="text-sm font-medium text-gray-600">
              Schritt {currentStepIndex + 1} / {WIZARD_STEPS.length}:{" "}
              {currentStep.label}
            </span>
            ...
            >
              <div className="p-4 border-b border-gray-100">
                ...
                <p className="text-xs text-gray-500 mt-1">
                  Fragen Sie mich zu Ihrem Stream
                </p>
              </div>
