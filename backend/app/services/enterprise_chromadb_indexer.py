"""
🏢 ENTERPRISE CHROMADB INDEXER - PRODUCTION READY
Enterprise-grade document indexing service with ChromaDB integration
"""
import logging
...
class EnterpriseChromaDBIndexer:
    ...
    async def initialize(self):
        """Initialize enterprise ChromaDB indexer"""
        if self.is_initialized:
            return
            
        try:
            logger.info("🏢 Enterprise ChromaDB Indexer initializing...")
            ...
            logger.info("✅ Enterprise ChromaDB Indexer ready")
            
        except Exception as e:
            logger.error(f"❌ Enterprise ChromaDB Indexer failed: {e}")
            raise
    ...
    async def index_file(self, file_id: str, db: AsyncSession) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"✅ Enterprise document indexing complete: {file_record.original_filename} ({len(chunks)} chunks)")
            ...
            logger.error(f"❌ Enterprise document indexing failed: {e}")
    ...
    async def remove_file_from_index(self, file_id: str, db: AsyncSession) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.collection is not None:
                try:
                    # Query ChromaDB for all chunks with this file_id
                    results = self.collection.get(
                        where={"file_id": file_id}
                    )
                    chunk_ids = results["ids"] if results and "ids" in results else []
                except Exception as e:
                    logger.warning(f"⚠️ Could not query ChromaDB for file {file_id}: {e}")
            else:
                logger.warning(f"⚠️ ChromaDB collection not initialized for file {file_id}")
            
            # Remove from ChromaDB
            if chunk_ids and self.collection is not None:
                try:
                    logger.info(f"🗑️ Removing {len(chunk_ids)} chunks from ChromaDB...")
                    self.collection.delete(ids=chunk_ids)
                    logger.info(f"✅ Removed {len(chunk_ids)} chunks from ChromaDB")
                except Exception as e:
                    logger.error(f"❌ Failed to remove chunks from ChromaDB: {e}")
                    # Continue with database update even if ChromaDB removal fails
