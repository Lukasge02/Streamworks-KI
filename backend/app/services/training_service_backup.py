...
class TrainingService:
    ...
    async def remove_from_chromadb(self, file_id: str) -> bool:
        """Remove a file from ChromaDB index with improved sync handling"""
        # Get file record
        query = select(TrainingFile).where(TrainingFile.id == file_id)
        result = await self.db.execute(query)
        file_record = result.scalar_one_or_none()
        ...
        if not file_record:
            logger.warning(f"⚠️ File record not found for {file_id}")
            return False
        ...
        try:
            ...
            # Enhanced ChromaDB removal - remove by multiple criteria
            removal_methods = [
                # Method 1: Remove by file_id metadata
                {"file_id": file_id},
                # Method 2: Remove by source (filename)
                {"source": file_record.filename},
                # Method 3: Remove by file_path
                {"source": file_record.file_path},
            ]
            
            # Also try processed_file_path if it exists
            if file_record.processed_file_path:
                removal_methods.append({"source": file_record.processed_file_path})
            ...
            for method in removal_methods:
                try:
                    ...
                    results = collection.get(where=method)
                    
                    if results['ids']:
                        collection.delete(ids=results['ids'])
                        total_removed += len(results['ids'])
                        logger.info(f"🗑️ Removed {len(results['ids'])} chunks using criteria: {method}")
                
                except Exception as method_error:
                    logger.warning(f"⚠️ Removal method {method} failed: {method_error}")
                    continue
            
            # Persist changes if any removals were made
            if total_removed > 0:
                rag_service.vector_store.persist()
                logger.info(f"✅ Total chunks removed from ChromaDB: {total_removed}")
            
            # Legacy removal method if chromadb_ids exists
            if hasattr(file_record, 'chromadb_ids') and file_record.chromadb_ids:
                try:
                    for doc_id in file_record.chromadb_ids:
                        await rag_service.delete_document(doc_id)
                    logger.info(f"🗑️ Legacy removal by chromadb_ids completed")
                except Exception as legacy_error:
                    logger.warning(f"⚠️ Legacy removal failed: {legacy_error}")
            ...
            # Clear chromadb_ids if it exists
            if hasattr(file_record, 'chromadb_ids'):
                file_record.chromadb_ids = None
            
            await self.db.commit()
            
            logger.info(f"✅ File removed from ChromaDB: {file_id} ({total_removed} chunks)")
            return True
