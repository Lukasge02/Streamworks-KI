...
Formatiere deine Antworten klar und strukturiert. Verwende Aufzählungen wo sinnvoll."""
    ...
    def __init__(self, use_reranking: bool = None, use_cache: bool = True):
        ...
        self._use_reranking = (
            use_reranking if use_reranking is not None else config.RAG_RERANK_ENABLED
        )
    ...
    def query(
        ...
        should_rerank = (
            use_reranking if use_reranking is not None else self._use_reranking
        )
        should_use_hybrid = (
            use_hybrid if use_hybrid is not None else config.RAG_HYBRID_ENABLED
        )
        ...
        if should_rerank and self.reranker:
            try:
                ...
                # Convert RankedResult back to dict format (handled as dicts because input was dicts)
                retrieved_chunks = [
                    {
                        "doc_id": r.get("doc_id"),
                        ...
                        "original_score": r.get("original_score", r.get("score")),
                        "metadata": r.get("metadata", {}),
                        "relevance_explanation": r.get("relevance_explanation"),
                    }
                    for r in ranked_results
                ]
                # Sort by score descending (RerankerService already does this, but being safe)
        ...
        if not is_truly_relevant:
            return QueryResult(
                ...
                warnings=[
                    "Keine relevanten Informationen für diese spezifische Frage gefunden"
                ],
                reranked=reranked,
            )
...
ANTWORT:""",
            ...
            sources.append(
                {
                    ...
                    or meta.get("filename", "Unbekannt"),
    ...
    def _classify_query(self, query: str) -> str:
        ...
        if any(word in query_lower for word in ["wie", "how", "anleitung", "schritt"]):
            ...
            word in query_lower
            for word in ["config", "konfigur", "einstell", "parameter"]
        ):
            return "configuration"
        elif any(
            word in query_lower
            for word in ["fehler", "problem", "error", "fix", "lösung"]
        ):
            return "troubleshooting"
        elif any(
            word in query_lower for word in ["was ist", "what is", "erkläre", "explain"]
        ):
            return "explanation"
        else:
            return "general"
