...
class VectorStore:
    ...
    def add_documents_batch(self, documents: List[Dict[str, Any]]) -> List[str]:
        ...
        for i, (doc, embedding) in enumerate(zip(documents, embeddings)):
            doc_id = self._generate_id(
                doc["content"], doc.get("metadata", {}).get("filename", f"doc_{i}")
            )
            point_id = int(hashlib.md5(doc_id.encode()).hexdigest()[:16], 16)
    ...
    def search(
        ...
        Args:
            ...
            filters: Metadata filters (category, doc_type, etc.)
            access_filter: Access control filter with:
                - doc_ids: List of allowed document IDs (only these are searched)
                - exclude_doc_ids: List of document IDs to exclude

        Returns:
            List of matches with content, metadata, and score
        """
        ...
        # Process standard metadata filters
        if filters:
            from qdrant_client.http import models
        ...
        # Process access control filters
        must_not_conditions = []
        should_conditions = []
        ...
        if access_filter:
            ...
            # Include only specific doc_ids (allowlist)
            if "doc_ids" in access_filter and access_filter["doc_ids"]:
                allowed_ids = access_filter["doc_ids"]
                # Match parent_doc_id OR doc_id in allowed list
                should_conditions.append(
                    models.FieldCondition(
                        key="parent_doc_id", match=models.MatchAny(any=allowed_ids)
                    )
                )
                should_conditions.append(
                    models.FieldCondition(
                        key="doc_id", match=models.MatchAny(any=allowed_ids)
                    )
                )

            # Exclude specific doc_ids (denylist)
            if "exclude_doc_ids" in access_filter and access_filter["exclude_doc_ids"]:
                excluded_ids = access_filter["exclude_doc_ids"]
                for excluded_id in excluded_ids:
                    must_not_conditions.append(
                        models.FieldCondition(
                            key="parent_doc_id",
                            match=models.MatchValue(value=excluded_id),
                        )
                    )
        ...
        if filter_parts:
            query_filter = models.Filter(**filter_parts)
    ...
    def delete_by_parent_id(self, parent_doc_id: str) -> int:
        ...
        try:
            ...
            if points_to_delete:
                ...
                print(
                    f"✅ Deleted {deleted_count} chunks for parent_doc_id: {parent_doc_id}"
                )
        ...
        try:
            ...
            while True:
                ...
                for point in points:
                    ...
                    parent_id = point.payload.get("parent_doc_id") or point.payload.get(
                        "doc_id"
                    )
                    if parent_id:
