...
class DocumentService:
    ...
    def process_file(
        ...
        **kwargs,
        ...
        if page_data and parsed_doc.doc_type.value == "pdf":
            ...
            chunks = enterprise_chunker.chunk(
                parsed_doc.content, strategy=ChunkingStrategy.SEMANTIC
            )
    ...
    def process_file_background(
        ...
        try:
            ...
            # 4. Extract intelligent metadata (Version, Date)
            import re
            ...
            # Determine category from filename prefix (IT_, HR_, etc)
            category_match = re.match(r"^([A-Z]+)_", filename)
            auto_category = category_match.group(1) if category_match else None
            final_category = explicit_category or auto_category
            ...
            # 5. Embed / Store with rich chunk metadata
    ...
    def process_text(
        self, text_content: str, filename: str, doc_type: str = "txt"
    ) -> Dict[str, Any]:
    ...
    def search(
        ...
        results = self.vector_store.search(
            query, limit=limit, score_threshold=score_threshold, filters=filters
        )

        if doc_type:
            results = [
                r for r in results if r.get("metadata", {}).get("doc_type") == doc_type
            ]
    ...
    def delete_document(self, doc_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            print(
                f"✅ Cascade delete: {chunks_deleted} chunks, file={'deleted' if file_deleted else 'not found'}"
            )
        ...
        try:
            ...
            for parent_id in orphaned_qdrant:
                try:
                    ...
                    report["errors"].append(
                        f"Failed to delete Qdrant entry {parent_id}: {e}"
                    )
            ...
            for doc_id in orphaned_minio:
                try:
                    ...
                    report["errors"].append(
                        f"Failed to delete MinIO file {doc_id}: {e}"
                    )
            ...
            all_cleaned = report["qdrant_cleaned"] == len(orphaned_qdrant) and report[
                "minio_cleaned"
            ] == len(orphaned_minio)
            report["in_sync"] = cleanup_successful and all_cleaned

            print(
                f"✅ Sync complete: {report['qdrant_cleaned']} Qdrant entries, {report['minio_cleaned']} MinIO files cleaned"
            )
    ...
    def get_consistency_report(self) -> Dict[str, Any]:
        ...
        try:
            ...
            report["all_consistent"] = (
                len(orphaned_qdrant) == 0 and len(orphaned_minio) == 0
            )
