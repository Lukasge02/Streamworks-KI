...
class EnterpriseChunker:
    ...
    def _chunk_semantic(self, text: str) -> List[Chunk]:
        ...
        if current_chunk.strip():
            chunks.append(
                self._create_chunk(
                    current_chunk.strip(), len(chunks), current_start, char_position
                )
            )
        ...
        for chunk in semantic_chunks:
            if len(chunk.content) <= self.max_chunk_size:
                ...
                sub_chunks = self._split_oversized(
                    chunk.content, chunk.metadata.start_char
                )
    ...
    def _chunk_fixed(self, text: str) -> List[Chunk]:
        ...
        while start < len(text):
            ...
            if end >= len(text):
                ...
                if chunk_text:
                    chunks.append(
                        self._create_chunk(chunk_text, len(chunks), start, len(text))
                    )
                break
            ...
            if chunk_text:
                chunks.append(
                    self._create_chunk(chunk_text, len(chunks), start, break_point)
                )
    ...
    def _split_oversized(self, text: str, start_offset: int) -> List[Chunk]:
        ...
        if current.strip():
            chunks.append(
                self._create_chunk(
                    current.strip(), 0, current_start, current_start + len(current)
                )
            )
