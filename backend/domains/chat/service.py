...
class ChatService:
    ...
    def process_message(self, message: str, session: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        merged_params = self.extractor.merge_params(session["params"], extraction)

        # Calculate completion
        completion = self._calculate_completion(
            extraction.job_type, merged_params, extraction.missing_required
        )

        # Generate response
        if extraction.missing_required:
            response = extraction.follow_up_question or self._generate_response(
                extraction, merged_params
            )
        else:
            response = (
    ...
    def _calculate_completion(
        self, job_type: str, params: Dict, missing: List[str]
    ) -> float:
    ...
    def _generate_response(self, extraction: StreamworksParams, params: Dict) -> str:
